...
// Use the same Supabase credentials as your main script
...
async function exportToCSV() {
    ...
    // Fetch all insights for the selected range, ignoring pagination
    let url = `/.netlify/functions/get-past-insights?limit=1000`; // High limit to get all
    if (dateRange.start && dateRange.end) {
        url += `&startDate=${dateRange.start.toISOString()}&endDate=${dateRange.end.toISOString()}`;
    }
    ...
    try {
        ...
        insights.forEach(row => {
            ...
            const insight = `"${row.insight_text.replace(/"/g, '""')}"`; // Escape double quotes
            csvContent += `${date},${insight}\r\n`;
        });
