...
// ... All event listeners in DOMContentLoaded are the same ...

// === All data fetching and helper functions are the same EXCEPT renderChart ===

/**
 * Renders a single chart instance.
 * NOW INCLUDES LOGIC FOR CREATING A GRADIENT FILL.
 */
function renderChart(canvasId, label, labels, data, hexColor, options) {
    ...
    // --- Create the gradient ---
    const gradient = ctx.createLinearGradient(0, 0, 0, 200); // Fades from top to bottom
    // Convert hex color to RGBA for the gradient
    const r = parseInt(hexColor.slice(1, 3), 16);
    const g = parseInt(hexColor.slice(3, 5), 16);
    const b = parseInt(hexColor.slice(5, 7), 16);
    gradient.addColorStop(0, `rgba(${r}, ${g}, ${b}, 0.4)`); // Start with a semi-transparent color
    gradient.addColorStop(1, `rgba(${r}, ${g}, ${b}, 0)`);   // Fade to fully transparent
    ...
    charts[canvasId] = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: label,
                data: data,
                backgroundColor: gradient, // Use the new gradient
                borderColor: hexColor,   // The line color remains solid
                borderWidth: 2,
                pointRadius: 3,
                pointBackgroundColor: hexColor,
                fill: true, // This enables the background fill
            }]
        },
        options: {
            ...options,
...
function renderAllCharts(data) {
    const commonOptions = {
        ...
        elements: { line: { tension: 0.4 } } // Slightly more curve
    };

    // New "Muted Neon" color palette
    const clarityColor = '#38bdf8'; // A vibrant sky blue
    const immuneColor = '#facc15';  // A rich gold/yellow
    const physicalColor = '#4ade80';// A bright green
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    signupForm.addEventListener('submit', async (event) => {
        ...
        else { authError.style.display = 'none'; alert('Success! Please check your email for a confirmation link.'); }
    });
    ...
    supabase.auth.onAuthStateChange((event, session) => {
        if (session) {
            ...
            loadRecentLogs(); fetchAndRenderCharts(7); fetchAndDisplayInsight(); fetchAndRenderInsightHistory();
        } else {
            appContainer.style.display = 'none'; authContainer.style.display = 'block';
        }
    });

    // UI TOGGLES & OTHER LISTENERS
    showSignupLink.addEventListener('click', (e) => {
        e.preventDefault(); loginForm.style.display = 'none'; signupForm.style.display = 'block';
        showSignupLink.style.display = 'none'; showLoginLink.style.display = 'block';
    });
    showLoginLink.addEventListener('click', (e) => {
        e.preventDefault(); signupForm.style.display = 'none'; loginForm.style.display = 'block';
        showLoginLink.style.display = 'none'; showSignupLink.style.display = 'block';
    });
    if (analyzeBtn) analyzeBtn.addEventListener('click', submitLog);
    if (logTextarea) {
        logTextarea.addEventListener('keydown', (event) => {
            if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); submitLog(); }
        });
    }
    ...
    if (btn7day) btn7day.addEventListener('click', () => {
        fetchAndRenderCharts(7); btn7day.classList.add('active'); btn30day.classList.remove('active');
    });
    if (btn30day) btn30day.addEventListener('click', () => {
        fetchAndRenderCharts(30); btn30day.classList.add('active'); btn7day.classList.remove('active');
    });
});
...
async function submitLog() {
    ...
    button.disabled = true; button.innerText = "Analyzing..."; spinner.style.display = 'inline-block';
    ...
    try {
        ...
        const response = await fetch('/.netlify/functions/analyze-log', {
            ...
            body: JSON.stringify({ log: entryText, sleep_hours: sleepHours ? parseFloat(sleepHours) : null, sleep_quality: sleepQuality ? parseInt(sleepQuality, 10) : null }),
        });
        if (!response.ok) {
        ...
        displayResults(newLog); await loadRecentLogs(); await fetchAndRenderCharts(7); await fetchAndDisplayInsight(); await fetchAndRenderInsightHistory();
        document.querySelector('#btn7day').classList.add('active'); document.querySelector('#btn30day').classList.remove('active');
    } catch (e) {
...
async function fetchAndDisplayInsight() {
    ...
    try {
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) return;
        const response = await fetch('/.netlify/functions/get-insight', { headers: { 'Authorization': `Bearer ${session.access_token}` } });
        if (!response.ok) throw new Error("Failed to fetch insight");
        const data = await response.json();
        if (data.insight) { insightTextElement.textContent = data.insight; insightTextElement.classList.remove('subtle-text'); } 
        else { insightTextElement.textContent = 'Not enough data to generate an insight yet.'; }
    } catch (e) {
...
async function fetchAndRenderInsightHistory() {
    ...
    try {
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) return;
        const response = await fetch('/.netlify/functions/get-past-insights', { headers: { 'Authorization': `Bearer ${session.access_token}` } });
        if (!response.ok) throw new Error("Failed to fetch insight history");
        const insights = await response.json();
        container.innerHTML = ''; 
        if (!insights || insights.length === 0) { container.innerHTML = '<p class="subtle-text">No saved insights yet.</p>'; return; }
        insights.forEach(insight => {
            const details = document.createElement('details'); details.classList.add('insight-item');
            const summary = document.createElement('summary'); summary.textContent = new Date(insight.created_at).toLocaleDateString();
            const p = document.createElement('p'); p.textContent = insight.insight_text;
...
function displayResults(result) {
    ...
    document.getElementById('log').value = ''; document.getElementById('sleep-hours').value = ''; document.getElementById('sleep-quality').value = '';
}

function renderLogTable(logs) {
    const tbody = document.querySelector('#logTable tbody');
    tbody.innerHTML = '';
    if (!logs || logs.length === 0) { tbody.innerHTML = `<tr><td colspan="6" style="text-align: center;">No entries found.</td></tr>`; return; }
    logs.forEach(logData => {
        const tr = document.createElement('tr');
        tr.addEventListener('click', () => openLogModal(logData));
        td(new Date(logData.created_at).toLocaleDateString(), tr); td(logData.Log, tr);
        td(logData.Clarity, tr, 'positive'); td(logData.Immune, tr, 'inverse'); td(logData.PhysicalReadiness, tr, 'positive');
        td(logData.Notes, tr);
...
function td(content, parent, scoreType = null) {
    const cell = document.createElement('td'); cell.textContent = content;
    if (scoreType) {
...
function resetUI() {
    const button = document.getElementById('analyzeBtn'); const spinner = document.getElementById('spinner');
    button.disabled = false; button.innerText = "Analyze Log"; spinner.style.display = 'none';
}