...
exports.handler = async (event, context) => {
    ...
    try {
        ...
        if (!supabaseUrl || !serviceRoleKey) {
            throw new Error("Critical Error: Supabase environment variables are not set.");
        }
        
        // Use a dedicated admin client initialized inside the handler
        ...
        for (const user of users) {
            ...
            for (const metricName in metrics) {
                ...
                if (isSignificantTrend && isStableData) { 
                    ...
                    const prompt = `A high-confidence downward trend was detected in a user's ${metricName} score...`;
