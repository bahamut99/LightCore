  ...
  return (
    <>
      ...
      <div style={{ position: 'absolute', top: '2rem', left: '2rem', width: '400px', color: '#00f0ff', background: 'rgba(10, 25, 47, 0.7)', border: '1px solid rgba(0, 240, 255, 0.2)', backdropFilter: 'blur(10px)', borderRadius: '0.5rem', padding: '1.5rem', fontFamily: "'Roboto Mono', monospace", fontSize: '14px', animation: 'fadeIn 0.5s ease-out', zIndex: 10 }}>
        ...
        {isGuide && <p className="hud-scroll" style={{ color: 'white', whiteSpace: 'pre-wrap', lineHeight: '1.6', fontStyle: 'italic', marginBottom: '1rem' }}>{item.current_state}</p>}
        <div className="hud-scroll" style={{ maxHeight: '200px', overflowY: 'auto', paddingRight: '1rem', marginBottom: '1rem' }}>
          {isLog && <p style={{ color: 'white', whiteSpace: 'pre-wrap', lineHeight: '1.6' }}>{item.ai_notes}</p>}
        ...
        <div style={{ borderTop: '1px solid rgba(0, 240, 255, 0.2)', paddingTop: '1rem' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
            ...
            {isNudge && item.suggested_actions?.map(action => <span key={action} style={{ background: 'rgba(255, 77, 77, 0.2)', padding: '0.25rem 0.75rem', borderRadius: '99px', fontSize: '12px' }}>{action}</span>)}
            {isGuide && item.suggestions?.map(s => <span key={s} style={{ background: 'rgba(0, 240, 255, 0.1)', padding: '0.25rem 0.75rem', borderRadius: '99px', fontSize: '12px' }}>{s}</span>)}
          </div>
        </div>
      </div>
    </>
...
const fragmentShader = `
  uniform float uHover; // 0.0 when not hovered, 1.0 when hovered
  uniform vec3 uColor;
  varying vec3 vNormal;
  void main() {
    float intensity = pow(0.7 - dot(vNormal, vec3(0.0, 0.0, 1.0)), 2.0);
    intensity += uHover * 0.5; // Additive glow on hover
...
function Locus({ onLocusClick }) {
  ...
  return (
    <>
      <mesh ref={meshRef} onClick={onLocusClick} onPointerOver={() => setHovered(true)} onPointerOut={() => setHovered(false)}>
...
function LogNode({ log, position, setSelectedItem, isSelected, setHoveredLog, isHovered }) {
  ...
  return (<mesh ref={meshRef} position={position} onClick={(e) => { e.stopPropagation(); setSelectedItem({ log, position }); }} onPointerOver={(e) => { e.stopPropagation(); setHoveredLog(log); }} onPointerOut={() => setHoveredLog(null)}><sphereGeometry args={[0.2, 32, 32]} /><meshStandardMaterial color={dynamicColor} metalness={0.95} roughness={0.1} emissive={dynamicColor} emissiveIntensity={isSelected || isHovered ? 0.5 : 0} /></mesh>);
}
...
function Constellation({ logs, setSelectedItem, selectedItem, setHoveredLog, hoveredLog }) {
  return useMemo(() => {
    ...
    return logs.map((log, index) => {
      ...
      return (<LogNode key={log.created_at} log={log} position={position} setSelectedItem={setSelectedItem} isSelected={selectedItem?.log?.created_at === log.created_at} setHoveredLog={setHoveredLog} isHovered={hoveredLog?.created_at === log.created_at}/>);
    });
