...
function Trends() {
  ...
  useEffect(() => {
    const cleanup = () => Object.values(chartInstances.current).forEach(chart => chart?.destroy());
    cleanup();
  ...
  function renderChart(canvasRef, label, labels, data, hexColor, options) {
    ...
    return new Chart(ctx, {
        ...
        data: {
            labels: labels,
            datasets: [{
                label, data, backgroundColor: gradient, borderColor: hexColor, borderWidth: 2,
                pointRadius: 3, pointBackgroundColor: hexColor, fill: true, tension: 0.4
            }]
        },
        options: { ...options, plugins: { ...options.plugins, title: { display: true, text: label, color: '#9CA3AF', font: { size: 16 } } } }
    });
  }
  ...
  return (
    <div className="card">
        ...
        {isLoading ? <p className="subtle-text">Loading charts...</p> : (
            <>
